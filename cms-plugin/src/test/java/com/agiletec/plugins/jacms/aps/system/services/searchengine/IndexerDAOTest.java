...
class IndexerDAOTest {
    ...
    @Test
    void shouldIndexAsTextWhenADeclaredTypeIsNotBackedByItsPlatformClass() throws Exception {
        // The dispatch is on the DECLARED SearchFieldType, so a custom attribute type may answer DATE or
        // NUMBER without extending DateAttribute/NumberAttribute. Narrowing to those classes must then be
        // guarded: the declaration says how the value is queried, not where it is read from.
        DeclaredDateTextAttribute pseudoDate = new DeclaredDateTextAttribute();
        pseudoDate.setName("when");
        pseudoDate.setType("CustomDate");
        pseudoDate.setIndexingType(IndexableAttributeInterface.INDEXING_TYPE_TEXT);
        pseudoDate.setText("some day");

        DeclaredNumberTextAttribute pseudoNumber = new DeclaredNumberTextAttribute();
        pseudoNumber.setName("howMany");
        pseudoNumber.setType("CustomNumber");
        pseudoNumber.setIndexingType(IndexableAttributeInterface.INDEXING_TYPE_TEXT);
        pseudoNumber.setText("a few");

        Document document = Assertions.assertDoesNotThrow(
                () -> newIndexerDAO().createDocument(newContent(pseudoDate, pseudoNumber)));

        Assertions.assertEquals("some day", document.get("en_when"));
        Assertions.assertEquals("a few", document.get("en_howMany"));
    }

    /** A custom type declaring itself a date without being a {@code DateAttribute}. */
    private static class DeclaredDateTextAttribute extends MonoTextAttribute {
    ...
    /** A custom type declaring itself a number without being a {@code NumberAttribute}. */
    private static class DeclaredNumberTextAttribute extends MonoTextAttribute {
