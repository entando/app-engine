...
public class GlobalLockManager implements IGlobalLockManager {
    ...
    private static final EntLogger log = EntLogFactory.getSanitizedLogger((GlobalLockManager.class));
    ...
    private static final String LOCK_LUA =
            "local created = redis.call('HSETNX', KEYS[1], '" + LOCK_PROP_TOKEN + "', ARGV[1]) " +
            "if created == 1 then " +
                    "redis.call('PEXPIRE', KEYS[1], ARGV[4]) " +
                    "redis.call('HSET', KEYS[1], '" + LOCk_PROP_CREATED_AT + "', ARGV[2]) " +
                    "redis.call('HSET', KEYS[1], '" + LOCK_PROP_DATA + "', ARGV[3]) " +
            "return ARGV[1] " +
            "end " +
            "return created ";
    ...
    public String tryLock(String key, String property, Duration duration) throws GlobalLockEntException {
        try {
            if (StringUtils.isNotBlank(key)) {
                ...
                String res = cmd.eval(LOCK_LUA, ScriptOutputType.VALUE, new String[]{key},
                        token,
