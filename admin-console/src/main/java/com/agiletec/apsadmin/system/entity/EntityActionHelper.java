...
import com.agiletec.aps.system.common.entity.SearchableAttributeRef;
...
public class EntityActionHelper extends BaseActionHelper implements IEntityActionHelper, BeanFactoryAware {
	...
	public EntitySearchFilter[] getAttributeFilters(AbstractApsEntityFinderAction entityFinderAction, IApsEntity prototype) {
		...
		List<SearchableAttributeRef> searchableAttributes = NestedBooleanSearchSupport
				.collectSearchable(prototype, entityFinderAction.getSearchKeyStrategy());
		for (SearchableAttributeRef ref : searchableAttributes) {
			...
			if (attribute instanceof ITextAttribute) {
			...
			} else if (SearchFieldType.TRISTATE == attribute.getSearchFieldType()) {
				...
				// Dispatched on the declared field type, so this branch no longer has to be ordered
				// before the boolean one to avoid being swallowed by it.
				EntitySearchFilter filterToAdd = this.buildThreeStateFilter(entityFinderAction, key);
				if (null != filterToAdd) {
					filters = this.addFilter(filters, filterToAdd);
				}
			} else if (SearchFieldType.BOOLEAN == attribute.getSearchFieldType()) {
	...
	public String[] getAttributeFilterFieldName(ApsEntity prototype, String attrName) {
		...
		} else if (SearchFieldType.BOOLEAN == attr.getSearchFieldType()
				|| SearchFieldType.TRISTATE == attr.getSearchFieldType()) {
